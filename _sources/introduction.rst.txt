...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/debussy_estampes.git
...
* `Claude Debussy – Estampes <https://github.com/DCMLab/debussy_estampes/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the first piece, *Pagodes*, has the following files:

-  ``MS3/l100-01_estampes_pagode.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l100-01_estampes_pagode.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l100-01_estampes_pagode.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l100-01_estampes_pagode.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l100-01_estampes_pagode.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/l100-01_estampes_pagode.tsv')
   notes = ms3.load_tsv('notes/l100-01_estampes_pagode.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/debussy_estampes/issues>`__ and feel free to fork and submit pull requests.
