...
Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first piece, *Pagodes*, has the following files:

-  ``MS3/l100-01_estampes_pagode.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/l100-01_estampes_pagode.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l100-01_estampes_pagode.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/l100-01_estampes_pagode.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
